function [tracksFinal,kalmanInfoLink,errFlag] = trackCloseGapsKalmanSparseSpatialAverage(...
...
% [JM] Averaged in space nearest neighbour

    if isfield(costMatrices(2).parameters,'avgSz')
        avgSz  = costMatrices(2).parameters.avgSz;
    else
        warning('avgSz not Defined !')
    end

    if isfield(costMatrices(2).parameters,'imsz')
        imsz   = costMatrices(2).parameters.imsz;
    else
        warning('imsz not Defined !')
    end

oPos  = convertDetectionFromDanuser(movieInfo);
nnMap = buildNnMap(oPos,imsz,avgSz); 
...
for iFrame = 1 : numFramesEff
    ...
    if (iFrame < numFramesEff)
        
        if (~isempty(coordFrame))
            
            %Compute the map for the specific frame
            mD = nnMap;
            
            %Get Coordinates current frame
            ix = sub2ind(imsz, round(coordFrame(:,2)), round(coordFrame(:,1)));
            
            % Get the averaged nn distance for each feature in this frame.
            nnDistAverage = mD(ix);
            
        end
        
    else %For last frame take second-to-end nnDistAverage
        nnDistAverage = max(nnDistAverage(:)) * ones(size(coordFrame(:,1)));
    end
    
    %store nearest neighbor distances in matrix
    nnDistLinkedFeat(~isnan(nnDistLinkedFeat(:,iFrame)),iFrame) = nnDistAverage;

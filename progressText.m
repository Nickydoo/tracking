...
% constants
nCharsBase = 9; % change this if changing output format
...
if fractionDone == 0
    ...
    printText = sprintf('%s%%03.0d%%%% done',text);
    initialText = sprintf('%s000%%%% done',text);
    finalText = sprintf('%s100%%%% done\n',text);
...
if percentDone == 100 % Task completed
    fprintf(1,fprintfExpressionFinal); % finish up
...
fprintf(1,fprintfExpression,percentDone);
